	...
	// -- Validation
		...
		public static void semanticValidation(String thingId, Model thing) {
			if(Directory.shaclValidation) {
				String shape = readFile(Directory.SHAPE_FILE);
				if(shape.isEmpty())
				...
				if(!report.contains(null, SH_CONFORMS, ResourceFactory.createTypedLiteral(true))) {
					System.out.println(thing.write(System.out,"TURTLE"));
					System.out.println(report.write(System.out,"TURTLE"));
					System.out.println("*************************************");
		...
		public static void syntacticValidation(Thing thing) {
			if(Directory.jsonSchemaValidation) {
				String schema = readFile(Directory.SCHEMA_FILE);
				if(schema.isEmpty()) 
